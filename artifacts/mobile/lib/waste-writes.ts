// Mirrored from the website's src/lib/waste-writes.ts.
//
// The BODY is byte-for-byte the website's: the single batched write, the 500-op
// guard, the snapshotted unit cost, the per-row waste percentage. Stock and its
// cost history cannot be allowed to diverge between the apps, and a partial
// commit here would deduct stock with no record of where it went.
//
// ONE line differs: wasteRecordsCollection and WasteScope are imported from
// lib/waste-scope.ts rather than from the website's hooks/useProjectWasteStats,
// which is a React file bound to the web Firebase provider. The two helpers are
// pure and were lifted across unchanged. Nothing else may be edited here —
// re-copy the body when the website's changes.
...
import { wasteRecordsCollection, type WasteScope } from "@/lib/waste-scope"
